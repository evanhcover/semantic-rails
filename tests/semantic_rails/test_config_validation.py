...
def test_jaffle_reference_package_has_release_authoring_profile():
    ...
    assert report["ok"] is True
    # Filter out semantic-collision warnings — these are a new detector
    # surface and the jaffle reference package has a handful of known
    # label-mirroring pairs (e.g. measure.jaffle.X + metric.sales.X with
    # the same label) that should be tightened in a follow-up authoring
    # pass. Other authoring-quality warnings still must be zero.
    non_collision_warnings = [
        w
        for w in report["warnings"]
        if not (isinstance(w, dict) and "semantic collision risk" in str(w.get("message", "")))
    ]
...
def test_validate_config_warns_on_semantic_collision(tmp_path: Path):
    ...
    collision_warnings = [
        ...
        if isinstance(w, dict) and "semantic collision risk" in str(w.get("message", ""))
    ]
    ...
    message = " ".join(str(w.get("message", "")) for w in collision_warnings)
    assert "measure.demo.revenue_a" in message and "measure.demo.revenue_b" in message
    assert "identical label" in message
