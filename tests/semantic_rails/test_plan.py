...
def test_plan_preserves_explicitly_blocked_primary_over_generic_fallback(runtime_factory) -> None:
    runtime = runtime_factory("jaffle_shop")
    try:
        best = plan_payload(
            runtime,
...
def test_plan_full_detail_returns_alternatives_and_blocked(runtime_factory) -> None:
    runtime = runtime_factory("jaffle_shop")
    try:
        payload = plan_payload(runtime, intent="top stores by revenue", detail="full", limit=3)
    finally:
        runtime.close()
    assert payload["status"] == "ok"
