...
def test_query_request_limits_clip_rows_for_duckdb(runtime_factory):
    ...
    DuckDB results post-fetch. The `statement_timeout_ms` field is
    ...
    try:
        ...
        # statement_timeout_ms is accepted (DuckDB best-effort, no error)
        # but the response now surfaces a warning so the caller learns the
        # limit was dropped. Without this, a 1-second timeout on a runaway
        # query silently completes — exactly the "half-fake contract" v2
        # called out.
        ...
        assert timeout_warnings, (
            "DuckDB cannot enforce statement_timeout_ms — the response "
            "must surface a STATEMENT_TIMEOUT_NOT_HONORED warning"
        )
        assert timeout_warnings[0]["details"]["engine"] == "duckdb"
        assert timeout_warnings[0]["details"]["statement_timeout_ms"] == 60_000
...
def test_runtime_reload_rebuilds_state_after_package_edit(tmp_path):
    ...
    and drop cached compilations so config edits take effect without
    ...
    try:
        ...
        assert len(runtime._compile_cache._items) >= 1
        ...
        assert len(runtime._compile_cache._items) == 0
