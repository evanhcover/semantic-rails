...
from scripts.smoke_public_demo import (
    SmokeFailure,
    _require_mcp_tools,
    _require_ok_payload,
    _require_try_page,
...
def test_public_demo_smoke_validators_accept_expected_contracts():
    assert (
        _require_ok_payload(
            "health",
            {"ok": True, "package": {"id": "jaffle_shop"}},
        ).detail
        == "ok=true package=jaffle_shop"
    )
    ...
    assert _require_mcp_tools(tools, expected_tools=13).detail == "13 tools"
    assert _require_try_page("Semantic Rails plan validate compile").ok is True


def test_public_demo_smoke_validators_reject_wrong_package_or_tool_count():
    with pytest.raises(SmokeFailure):
        _require_ok_payload("health", {"ok": True, "package": {"id": "private_package"}})

    with pytest.raises(SmokeFailure):
        _require_mcp_tools({"result": {"tools": [{"name": "capabilities"}]}}, expected_tools=13)
