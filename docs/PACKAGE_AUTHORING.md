...
This guide is the narrative authoring guide. For per-attribute reference (every
field, type, default, status, error code), see
[website/docs/authoring-reference.html](../website/docs/authoring-reference.html).
...
## Quickstart: `init` and single-file packages

The fastest way to a working package is the scaffold:

```bash
uv run semantic-rails init --output ./my_pkg --package-id my_pkg
uv run semantic-rails validate-config --path ./my_pkg/package.yml
```

`init` writes a **single-file** package — one `package.yml` holding `package`,
`defaults`, `graph`, `models`, and `metrics` blocks, plus a `data/seed_example.sql`
seed. Single-file packages are loaded by pointing `--path` at the **file**, not
the directory:

```bash
uv run semantic-rails catalog --path ./my_pkg/package.yml --verbosity summary
uv run semantic-rails plan --path ./my_pkg/package.yml --intent "monthly revenue"
```

Pointing `--path` at the directory fails with `graph.yml is missing — found a
single-file .../package.yml; pass --path .../package.yml to load it`, because a
directory source is interpreted as the split layout below.

`validate-config` also writes a `.compiled/manifest.json` next to the package
(sibling `.compiled/` directory for single-file packages). The manifest holds
`package_id`, a content `fingerprint` of the source, and pre-rendered catalog
variants the runtime can serve without recompiling; a stale fingerprint falls
back to live compute. Pass `--no-manifest` to skip writing it.

Single-file packages can still have sibling `examples/` and `tests/` directories
next to the `package.yml` — `run-examples`, `test-package`, and `check` pick them
up from the file's parent directory, and `build-package` bundles them (plus the
referenced seed SQL) into the artifact.

**Exploding into a directory package.** When the file grows, copy each top-level
block into the split layout below. Two rules apply to directory packages:
...
For per-attribute YAML reference — every field, type, default, status, error
code, and runnable example — see
[Authoring Reference](../website/docs/authoring-reference.html).
