...
Hosted deployments that scale horizontally should share warm compilations
across workers. The cache contract is the `CompiledSqlCache` protocol in
`semantic_rails/cache.py`. Implement it against Redis, Memcached, or any
shared store, then inject it at startup with
`runtime.set_compile_cache(cache)`. The setter validates the protocol at
injection time and swaps the backend under the runtime's internal cache
lock, so misconfigured deployments fail fast instead of corrupting the
cache at request time. Cache keys are stable hex digests and cached
values are JSON-serialisable, so no escaping or runtime-specific
transport is required. The protocol-based seam keeps the OSS local
experience self-contained while allowing a hosted product to add a
shared backend without forking the runtime.
