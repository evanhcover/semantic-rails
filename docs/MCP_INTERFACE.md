...
`GET /mcp` returns `405` because the public demo does not require server-initiated SSE.
When `SEMANTIC_RAILS_API_KEYS` (or `SEMANTIC_RAILS_API_KEY_FILE`) is configured, `/mcp` requires
the same bearer API key as `/api/v1/*` and returns `401` otherwise; the hosted demo leaves keys
unset, which is what makes the endpoint anonymous.
Clients must send:
...
The anonymous endpoint is fixed to synthetic `jaffle_shop` data. It accepts no uploads or external
warehouse credentials, caps executed rows at 100 and segment previews at 25, limits request bodies
to 64 KiB, and applies edge rate and response-deadline controls. To keep MCP context small,
stay on the defaults (`minimal` for validate/compile/execute, `summary` for catalog) and reach
for `compact`/`full` only when you need explain plans or descriptive rows — measured sizes for
every tier are in the tables below.
...
try:
    tools = adapter.list_tools()       # ~16KB, paid once at connect time
    # Loop position 0: cheapest orientation — ~10.5KB envelope.
    capabilities = adapter.call_tool("capabilities", {})
    # Loop position 1: counts + flat ID list per kind — ~23KB.
...
- `capabilities` (loop position 0 — cheapest orientation, ~10.5KB)
- `catalog` (loop position 1 — counts + IDs at `verbosity=summary`, ~23KB)
- `discover`
- `inspect`
- `build-options`
- `valid-values`
...
Cold-start budget: `tools/list` itself is ~16KB (measured 15,882 bytes against the
hosted jaffle demo) and is paid once at connect time, before loop position 0. To keep it
that size, the IR cheat-sheet and the full Query-IR time-block schema ship once on the
`validate` tool description; `compile`, `execute`, and the other IR-accepting tools point
at it instead of repeating it. All byte sizes in this document were measured against the
bundled `jaffle_shop` package and will vary with package size.

### Catalog Verbosity Tiers

`catalog` accepts four `verbosity` values, sized by use case:

| Verbosity | Shape | Size (jaffle, measured) | Use case |
|---|---|---|---|
| `summary` (default) | counts + flat ID list per kind + capabilities | ~23KB | Cold-start orientation |
| `minimal` | skeleton rows `{id, kind, name, label, available}` + counts | ~79KB | "Show me what exists with labels" |
| `compact` | descriptive rows capped at 200/kind + counts + counts_total | ~472KB | Browsing with descriptions |
| `full` | uncapped rows + alias_index + aliases | ~2.3MB | Debugging, code-gen, exhaustive scans |
...
uv run semantic-rails mcp stdio --path /path/to/package
```

Legacy local HTTP JSON-RPC plus SSE readiness endpoint:

```bash
uv run semantic-rails mcp http --package jaffle_shop --host 127.0.0.1 --port 8091
curl -s http://127.0.0.1:8091/health
curl -s -N http://127.0.0.1:8091/sse
```
...
Cursor/Codex-style hosts can use the same stdio command. Hosts that support remote Streamable HTTP
can point directly at `https://semantic-rails.com/mcp`; local compatibility testing can use
`http://127.0.0.1:8091/mcp` after starting the legacy HTTP transport.

## Optional FastMCP Runtime

Importing `semantic_rails.mcp` never imports an external MCP package. If a host wants to mount the
adapter into a concrete MCP server, call `create_optional_fastmcp_server(adapter)` only in that host
boundary. That function imports `mcp.server.fastmcp.FastMCP` locally and raises a clear runtime
error when the optional package is not installed.
