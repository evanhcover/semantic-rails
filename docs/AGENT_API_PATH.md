...
- `execute` is the MCP tool name (HTTP path `/api/v1/query`, CLI verb `semantic-rails query`) and
  executes the compiled request in the local/customer-side runtime. A hosted v0 service should
  stay compile/validate/discovery-first until warehouse execution is productized.
