...
Semantic Rails is the public website name for this repo. The API itself is still served by the
`semantic-rails` runtime and CLI.
...
uv run semantic-rails init --output ./my_semantic_package --package-id my_semantic_package
...
- `statement_timeout_ms` — Snowflake enforces this via session-level
  `STATEMENT_TIMEOUT_IN_SECONDS`. DuckDB accepts the value but does not
  enforce a hard timeout (DuckDB has no portable timeout pragma); a
  hosted operator who needs hard DuckDB deadlines can subclass the
  adapter and drive `Connection.interrupt()` from a watchdog.
- `max_rows` — applied post-fetch as a result-set cap across all
  warehouses.
