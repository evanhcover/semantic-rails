...
class DuckDBAdapter(WarehouseAdapter):
    ...
    def query(self, sql: str, *, limits: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        if timeout_s > 0:
            # DuckDB has no statement_timeout pragma; the closest portable
            # equivalent is the connection-level interrupt fence applied
            # via signal alarms. We avoid that here because it is not
            # safe under threaded ASGI hosting — instead we surface the
            # caller's intent and rely on the post-fetch row cap. A
            # production hosted deployment that needs hard timeouts can
            # subclass and use DuckDB's `Connection.interrupt()` from a
            # watchdog thread.
            pass
