...
from .request_context import context_from_policy_context, emit_audit_event, request_context_payload
...
TOOL_DEFINITIONS: tuple[ToolDefinition, ...] = (
    ToolDefinition(
        ...
        description=(
            ...
            "Costs ~10KB; tells you which IR expression kinds the "
    ...
    ToolDefinition(
        ...
        description=(
            ...
            "(~22KB), right for orientation. Bump to 'compact' for row "
...
def _policy_context_payload(payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    return context_from_policy_context(
        policy_context, request_id=_clean_request_id(raw_payload.get("request_id"))
    ).to_policy_context()
...
class SemanticLayerMCPAdapter:
    ...
    def read_resource(self, uri: str) -> dict[str, Any]:
        if uri == "semantic-rails://capabilities":
            ...
            payload = self._envelope(
                {"catalog": catalog_payload(self.runtime, view="summary", verbosity="compact")},
                request_id="",
                started_at=time.perf_counter(),
            )
        elif uri == "semantic-rails://catalog/full":
            payload = self._envelope(
                {"catalog": catalog_payload(self.runtime, view="summary", verbosity="full")},
                request_id="",
                started_at=time.perf_counter(),
            )
        else:
    ...
    def _success(
        ...
        out.setdefault(
            ...
            request_context_payload(
                context_from_policy_context(
                    _policy_context_payload(arguments), request_id=str(out.get("request_id", ""))
                )
            ),
        )
        emit_audit_event(
            ...
            request_context=dict(out.get("request_context", {}) or {}),
    ...
    def _catalog_for(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        # Manifest only covers unfiltered, default-context catalogs. Filters
        # and non-default policy context fall through to live compute.
        unfiltered = not (kind or search or entity)
        default_context = not policy_context.get("environment") and not policy_context.get(
            "audience"
        )
        if unfiltered and default_context:
            cached = self.runtime.manifest_catalog(view=view, verbosity=verbosity)
    ...
    def _handle_segment_validate(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self._guarded(
            arguments, lambda args: self.runtime.segment_validate(str(args.get("segment_id", "")))
        )

    def _handle_segment_explain(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return self._guarded(
            arguments, lambda args: self.runtime.segment_explain(str(args.get("segment_id", "")))
        )
...
def create_optional_fastmcp_server(
    ...
    *,
    ...
    """Create a FastMCP server if the optional MCP package is installed.

    The import is intentionally local so importing semantic_rails.mcp never
    requires an external MCP runtime.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "Install an MCP runtime that provides mcp.server.fastmcp.FastMCP to create a network server."
        ) from exc
