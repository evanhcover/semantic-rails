...
* Lowercase-alphanumeric normalization and tokenization primitives
  (``_norm``, ``_tokenize``) — defined here because the catalog token index
  is their primary client. ``metadata.py`` re-imports them for general use.
...
def _derive_discovery_tokens(name: str, label: str, description: str) -> list[str]:
    """Derive discovery tokens from an object's name/label/description.

    Replaces the former author-provided ``search_terms`` / ``examples`` lists.
    Splits CamelCase boundaries (so ``CustomerSegmentMembership`` becomes
    ``customer segment membership``, not the false-positive substring
    ``customersegmentmembership``), tokenizes on non-alphanumeric,
    lowercases, dedupes, and strips common stopwords.
    """
    stopwords = {"the", "a", "an", "of", "and", "or", "to", "for", "in", "on", "by", "with", "is"}
    tokens: list[str] = []
    seen: set[str] = set()
    for source in (name or "", label or "", description or ""):
        camel_split = _CAMEL_BOUNDARY_RE.sub(" ", str(source))
        for tok in re.findall(r"[a-z0-9]+", camel_split.lower()):
...
def _tokenize(value: str) -> list[str]:
    return [_norm(part) for part in re.split(r"[^A-Za-z0-9]+", str(value or "")) if _norm(part)]
...
def _catalog_token_index(config: PackageConfig) -> set[str]:
    """Cache-free union of all queryable tokens from labels/names/search_terms.
    ...
    for collection in collections:
        for row in collection:
            for attr in ("id", "name", "label", "description"):
                tokens.update(_tokenize(str(getattr(row, attr, "") or "")))
            tokens.update(
                _derive_discovery_tokens(
            ...
            for topic in list(getattr(row, "topics", []) or []):
                tokens.update(_tokenize(str(topic)))
    tokens.discard("")
    return tokens


def _catalog_token_doc_freq(config: PackageConfig) -> dict[str, int]:
    ...
    for _, collection in collections:
        for row in collection:
            ...
            for attr in ("name", "label", "description"):
                obj_tokens.update(_tokenize(str(getattr(row, attr, "") or "")))
            obj_tokens.update(
                _derive_discovery_tokens(
                    str(getattr(row, "name", "") or ""),
                    str(getattr(row, "label", "") or ""),
            ...
            for topic in list(getattr(row, "topics", []) or []):
                obj_tokens.update(_tokenize(str(topic)))
            obj_tokens.discard("")
...
def _token_idf_weight(token: str, doc_freq: dict[str, int] | None) -> float:
...
def _intent_passes_relevance_floor(intent: str, catalog_tokens: set[str]) -> tuple[bool, list[str]]:
...
def _intent_passes_grounding_floor(
    intent: str, catalog_tokens: set[str], *, weak_tokens: set[str]
) -> tuple[bool, list[str]]:
