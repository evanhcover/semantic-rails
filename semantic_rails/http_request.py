...
def _safe_request_context_payload(payload: Mapping[str, Any] | None) -> dict[str, Any]:
    if not isinstance(payload, Mapping):
        return {}
    policy_context = payload.get("policy_context")
    if not isinstance(policy_context, Mapping):
        return {}
    return {"policy_context": dict(policy_context)}
...
def policy_context_payload(
    ...
    if context is not None:
        policy_context.update(context.to_policy_context())
...
def query_payload(
    ...
    if "policy_context" in query or policy_context:
        query_policy_context = _object_payload(
