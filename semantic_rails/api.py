...
class AppState:
    def __init__(self, package_id: str):
        self.runtime = Runtime(package_id)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _prepare_request(self, payload: dict[str, Any] | None = None) -> None:
        ...
        self._request_context = self._service().request_context(
            headers, payload=payload or {}, request_id=self._request_id
        )
...
def serve(package_id: str, host: str = "127.0.0.1", port: int = 8090) -> None:
    ...
    print(f"semantic-rails server running on http://{host}:{port} package={package_id}")
    httpd.serve_forever()
