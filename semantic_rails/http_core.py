...
class SemanticHTTPService:
    ...
    def request_context(
        ...
        *,
        ...
        return resolver.resolve(
            headers,
            payload=_safe_request_context_payload(payload),
    ...
    def capabilities_payload(self) -> dict[str, Any]:
        catalog = catalog_payload(self.runtime)
        ...
        *,
        ...
        if method == "POST":
            if context is not None:
                payload = {**payload, "policy_context": policy_context_payload(payload, context)}
            return self._handle_post(route, payload, context=context)
    ...
    def _handle_post(
        self, route: str, payload: Mapping[str, Any], *, context: RequestContext | None
    ) -> tuple[dict[str, Any], int]:
        if route == "/catalog":
            return {
                "ok": True,
        ...
        if route == "/discover":
            return {
                ...
                **discover_payload(
                    ...
                    partial_query=query_payload(payload, context)
        ...
        if route == "/validate":
            return {"ok": True, **self.runtime.validate(query_payload(payload, context))}, 200
        if route == "/compile":
            return {"ok": True, **self.runtime.compile(query_payload(payload, context))}, 200
        if route == "/query":
            return {"ok": True, **self.runtime.query(query_payload(payload, context))}, 200
        if route == "/segment-validate":
            return {
                "ok": True,
                **self.runtime.segment_validate(str(payload.get("segment_id", ""))),
            }, 200
        if route == "/segment-explain":
            return {
                "ok": True,
                **self.runtime.segment_explain(str(payload.get("segment_id", ""))),
            }, 200
        if route == "/segment-preview":
            return {
                "ok": True,
                **self.runtime.segment_preview(
                    str(payload.get("segment_id", "")), limit=int(payload.get("limit", 50))
                ),
            }, 200
        if route == "/valid-values":
            return {
                ...
                **valid_values_payload(
                    ...
                    query=query_payload(payload, context)
                    if payload.get("query") or payload.get("policy_context")
                    else None,
                    search=str(payload.get("search", "")),
