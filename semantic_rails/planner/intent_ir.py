...
def _subject_candidates(config: Any, terms: set[str]) -> list[ResolvedTerm]:
    ...
    from ._base import _score  # local to avoid bloating module top.

    content_terms = {term for term in terms if term not in _SUBJECT_STOPWORDS}
    if not content_terms:
        return []
    raw: list[tuple[int, str, str, str]] = []
    for row in config.measures:
        score = _score(row, content_terms)
        if score <= 0:
            continue
...
def parse_intent(runtime: Any, intent: str) -> IntentIR:
    ...
    target_terms = tuple(_target_measure_terms(text, terms))
    ...
    subjects = _subject_candidates(config, terms)
