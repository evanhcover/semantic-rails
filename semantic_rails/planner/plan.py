...
def plan_payload(
    ...
    *,
    ...
    if intent_str:
        ...
        catalog_tokens = _catalog_token_index(runtime.config)
    ...
    fallback_rows = fallback_drafts(
        runtime,
        intent=intent,
        partial_query=partial_query,
        limit=max(1, int(limit or 1)),
    )
    distinct_fallback_rows = [
        row for row in fallback_rows if _query_key(row[0].query) not in primary_query_keys
    ]
    if result.draft is None or detail_level in {"full", "debug"}:
        draft_rows.extend(distinct_fallback_rows)
    ...
    if (
        ...
        for draft, pattern in distinct_fallback_rows:
    ...
    ready = best_ok and time_why is None and conversion_why is None
    payload = {
        "plan_version": _VERSION,
        "intent": intent,
        "intent_ir": intent_ir.to_dict(),
...
def _conversion_intent_why(
    ...
    if not markers:
        return None
    if _query_contains_conversion(runtime, query):
        return None
    conversion_metrics = [
    ...
    return {
        ...
        "message": (
            "The intent reads as a conversion/funnel question, but "
            "best.query_ir contains no conversion expression — executing it "
            "as-is would answer a different question."
        ),
        "details": {
            "path": "select",
