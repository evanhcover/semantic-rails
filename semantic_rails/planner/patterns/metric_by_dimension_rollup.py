...
def _match(runtime: Any, text: str, terms: set[str]) -> RuntimeCompositionDraft | None:
    ...
    if temporal_role:
        ...
        if (
            is_top
            and group_by
            and not _explicit_grain(text)
            and not _implied_window_grain(lowered)
            and not _time_bounds_from_text(text)
        ):
            # "top stores by revenue" ranks stores overall. Keeping the
            # defaulted month grain would rank store-months instead, so the
            # bucket only stays when the intent carries a real time cue.
            time_spec = None
