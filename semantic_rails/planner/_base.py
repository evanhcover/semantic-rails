...
def _score(row: Any, terms: Iterable[str]) -> int:
    text_tokens = set(_tokens(_object_text(row)))
    primary_tokens = set(
        _tokens(
            " ".join(
                [
    ...
    score = 0
    for term in terms:
        if term in text_tokens or term in _object_text(row):
            score += 1
...
def _preferred_measure(config: Any, terms: Iterable[str]) -> Any | None:
    ...
    for row in config.measures:
        ...
        score -= sum(2 for item in penalties if item in text and item not in term_set)
...
def _preferred_metric(config: Any, terms: Iterable[str]) -> Any | None:
    ...
    for metric_id in preferred_ids:
        ...
        if match is not None:
            return match
    return _metric(config, term_set)
...
def _requested_grouping_terms(text: str) -> list[str]:
    ...
    return [
        term.strip()
        for term in re.split(r"\s*(?:,| and | & )\s*", by_match.group(1).strip())
        if term.strip()
    ]
