...
def _compiled_package_warnings(config, source_path: Path) -> list[str]:
    warnings: list[str] = []
...
def _semantic_collision_warnings(config, source_path: Path) -> list[str]:
    """Flag pairs of public objects whose label / name overlap enough
    that the MCP `discover` ranker may surface them indistinguishably.

    Signals (per pair):
      1. Identical normalised label (strongest signal).
      2. Label token overlap ≥ 80% with ≥ 2 shared tokens.
      3. Identical normalised name.

    Pair scope: same-kind pairs (measure↔measure, metric↔metric, etc.)
    plus cross-kind measure↔metric (both surface as the same kind in
    `discover`). Excludes ID-typed dimensions (auto-generated from
    primary keys; share labels by construction) and measure ↔
    auto-published-metric pairs (the publish contract intentionally
    shares `name`).
    """
    from itertools import combinations

    from .metadata_parts.relevance import _norm
    ...
    def _label_tokens(value: str) -> frozenset[str]:
        ...
        return frozenset(
            ...
            if _norm(part) and len(_norm(part)) >= 2
        )
    ...
    for measure in config.measures:
        if measure.name:
            measure_names[measure.name] = measure.id
    for recipe in config.metric_recipes:
        if recipe.name and recipe.name in measure_names:
            auto_pairs.add(tuple(sorted((measure_names[recipe.name], recipe.id))))
    ...
    for kind, collection in (
        ...
        for obj in collection:
            if kind == "dimension" and str(getattr(obj, "semantic_kind", "") or "").lower() == "id":
                continue
            items.append(
                {
                    "kind": kind,
                    "id": obj.id,
                    "label_norm": _norm(getattr(obj, "label", "") or ""),
                    "label_tokens": _label_tokens(getattr(obj, "label", "") or ""),
                    "name_norm": _norm(getattr(obj, "name", "") or ""),
                }
            )
    ...
    for a, b in combinations(items, 2):
        if a["kind"] != b["kind"] and {a["kind"], b["kind"]} != {"measure", "metric"}:
            continue
        pair_key = tuple(sorted((a["id"], b["id"])))
        if pair_key in seen_pairs or pair_key in auto_pairs:
            continue
        signals: list[str] = []
        if a["label_norm"] and a["label_norm"] == b["label_norm"]:
            signals.append("identical label")
        elif a["label_tokens"] and b["label_tokens"]:
            shared = a["label_tokens"] & b["label_tokens"]
            union = a["label_tokens"] | b["label_tokens"]
            if union and len(shared) / len(union) >= 0.8 and len(shared) >= 2:
                signals.append(f"label token overlap {len(shared)}/{len(union)}")
        if a["name_norm"] and a["name_norm"] == b["name_norm"]:
            signals.append("identical name")
        if not signals:
            continue
        seen_pairs.add(pair_key)
        warnings.append(
            f"{source_path}: semantic collision risk between "
            f"{a['kind']} {a['id']} and {b['kind']} {b['id']} — "
            f"{'; '.join(signals)}. "
            f"MCP `discover` may rank them indistinguishably. Differentiate "
            f"`label` or `name` on one of them."
        )
    return warnings
...
def parse_config_report(
    ...
    if not errors:
        ...
        warnings = [
            _error_payload("AUTHORING_PROFILE_WARNING", message)
            for message in _compiled_package_warnings(config, Path(ref.source_path))
        ]
    report = {
        "ok": not errors,
