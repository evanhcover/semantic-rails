...
def _relevance_score(
    ...
    *,
    ...
    if any(_norm(field) == terms_norm for field in exact_fields if field):
        score += 60.0
        reasons.append("exact name match")

    def _contains_token(values: Iterable[str], token: str) -> bool:
        return any(token and token in _norm(value) for value in values if value)

    derived_tokens = _derive_discovery_tokens(
        str(candidate.get("name", "") or ""),
        str(candidate.get("label", "") or ""),
        str(candidate.get("description", "") or ""),
    )
    if _contains_token(derived_tokens, terms_norm) or _contains_token(
        candidate.get("topics", []), terms_norm
    ):
        score += 25.0
    ...
    for token in tokens:
        ...
        for candidate_token in token_variants:
            if _contains_token(
                [candidate.get("label", ""), candidate.get("name", "")], candidate_token
            ):
                score += 12.0 * idf_weight
                reasons.append(f"label/name matched '{candidate_token}'")
            if _contains_token(derived_tokens, candidate_token):
                score += 10.0 * idf_weight
                reasons.append(f"derived-token matched '{candidate_token}'")
            if _contains_token(candidate.get("topics", []), candidate_token):
                score += 8.0 * idf_weight
                reasons.append(f"topic matched '{candidate_token}'")
            if _contains_token([candidate.get("description", "")], candidate_token):
                score += 6.0 * idf_weight
                reasons.append(f"description matched '{candidate_token}'")
            if _contains_token(
                candidate.get("comparison_peers", []), candidate_token
            ) or _contains_token(candidate.get("clock_variants", []), candidate_token):
                score += 6.0 * idf_weight
    ...
    if requested_grain:
        title = " ".join([str(candidate.get("label", "")), str(candidate.get("name", ""))]).lower()
...
def discover_payload(
    ...
    *,
    ...
    if enforce_scope and str(terms or "").strip():
        ...
        catalog_tokens = _catalog_token_index(config)
    ...
    token_doc_freq: dict[str, int] | None = (
        _catalog_token_doc_freq(config) if enforce_scope else None
    )
