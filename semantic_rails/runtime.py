...
def _adapter_query(adapter: Any, sql: str, *, limits: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if accepts_limits:
        return list(adapter.query(sql, limits=limits))
    return list(adapter.query(sql))
...
class Runtime:
    ...
    def set_compile_cache(self, cache: CompiledSqlCache) -> None:
        ...
        Hosted operators use this to inject a shared backend (Redis,
        Memcached, etc.) so warm compilations don't have to repeat on
        every worker. The runtime continues to keep the cache as the
        underscore-prefixed `_compile_cache` attribute internally — this
        setter is the public extension seam.
    ...
    def reload(self) -> dict[str, Any]:
        ...
        with serving traffic: each call rebuilds the runtime state under
        `_cache_lock`. In-flight requests that already hold a reference
        to the previous compiled plan finish on the old plan; subsequent
        compiles use the refreshed config.
        ...
        with self._cache_lock:
            ...
            self._compile_cache = LruCompiledSqlCache(
                maxsize=int(os.environ.get("SEMANTIC_RAILS_COMPILE_CACHE_SIZE", "512"))
            )
        return {
            ...
            limits_warnings.append(
                {
                    ...
                    "details": {
                        ...
                        "statement_timeout_ms": int(limits.get("statement_timeout_ms", 0) or 0),
                    },
                }
            )
        try:
    ...
    def segment_validate(self, segment_id: str) -> dict[str, Any]:
        started = time.perf_counter()
        try:
    ...
    def segment_explain(self, segment_id: str) -> dict[str, Any]:
        normalized = normalize_segment(self.config, segment_id)
        derived_query = build_segment_query(normalized, include_preview_dimensions=True)
    ...
    def segment_preview(self, segment_id: str, *, limit: int = 50) -> dict[str, Any]:
