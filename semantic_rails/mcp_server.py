...
def handle_jsonrpc_message(
    adapter: SemanticLayerMCPAdapter, message: dict[str, Any]
) -> dict[str, Any] | None:
    ...
    try:
        if method == "initialize":
            ...
            result = _tool_content(
                adapter.call_tool(str(params.get("name", "")), dict(raw_arguments))
            )
        elif method == "resources/list":
            result = {"resources": adapter.list_resources()}
        elif method == "resources/read":
            resource = adapter.read_resource(str(params.get("uri", "")))
            result = {
                "contents": [
                    {
                        "uri": resource["uri"],
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if route == "/sse":
                context = context_from_headers(
                    cast(Mapping[str, Any], self.headers), request_id=_request_id(self)
                )
                return _send_sse(
                    self,
                    [
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                if isinstance(payload, list):
                    if not payload:
                        ...
                        for item in payload:
                            if not isinstance(item, dict):
                                ...
                                continue
                            row = handle_jsonrpc_message(adapter, item)
                            if row is not None:
                                batch.append(row)
                    ...
                    response = handle_jsonrpc_message(adapter, payload)
            except json.JSONDecodeError as exc:
                return _send_json(
