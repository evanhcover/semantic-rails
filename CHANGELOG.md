...
- Stateless MCP Streamable HTTP served at `/mcp` by the ASGI app, protocol `2025-11-25`: JSON
  responses, `202` notifications, Origin validation, 64 KiB body cap, and a public-demo policy
  (`SEMANTIC_RAILS_PUBLIC_DEMO=1`) that pins the runtime to the bundled synthetic `jaffle_shop`
  package and bounds execute/segment-preview/valid-values responses.
- Cloudflare Workers deployment (`deploy/cloudflare/`) serving the static website and proxying
  `/api/*` + `/mcp` to a containerized backend with per-IP edge rate limits, request deadlines,
  and a canonical-host redirect; deploys to semantic-rails.com on push to `main`.
- Live "Try" page and hosted-MCP page on the website driving the real
  plan → validate → compile → query loop against the demo backend.
...
- Website: hero leads with the agent-runtime wedge, one-pane competitor diff lists the
  architectural rows that do not depend on the question slice, and the evidence page deflects
  the three common conflations (OSI, MetricFlow/Cube/Malloy/SV, dbt SL Cloud).
...
- Website: form fields now floor at 16px under 640px viewports so iOS Safari stops
  auto-zooming the try-page editor.
...
- Demo-mode caps now extend to `valid-values` live queries (`limit`/`offset` are clamped on both
  the HTTP and MCP surfaces).
- Cached `/api/v1/catalog` and `/api/v1/capabilities` responses at the edge now carry
  `Vary: Origin` so one origin's CORS header is never replayed to another.
