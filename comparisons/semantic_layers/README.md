...
| Cube | `1.6.32` | executed | 6 native, 4 workaround, 6 precomputed |
...
- `cube/`
  Minimal Cube Core project on the shared DuckDB dataset.
- `malloy/`
...
3. Execute the external runnable layers:
...
4. Rebuild the Snowflake trial pack with the default `semantic_views_trial` connection:
...
5. Regenerate the shared contracts:
